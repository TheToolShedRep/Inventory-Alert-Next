...
export async function POST(req: Request) {
  ...
  if (action === "purchased") {
    ...
    // ✅ ADDED: rebuild Shopping_List immediately after purchase/set
    try {
      const internalKey = process.env.INTERNAL_API_KEY || "";
      const origin = new URL(req.url).origin;
      const reorderRes = await fetch(`${origin}/api/inventory/reorder-check`, {
        method: "GET",
        headers: internalKey ? { "x-api-key": internalKey } : {},
        cache: "no-store",
      });

      if (!reorderRes.ok) {
        const preview = await reorderRes.text();
        console.error("❌ reorder-check refresh failed after purchase", {
          status: reorderRes.status,
          preview: preview.slice(0, 200),
        });
      }
    } catch (err: any) {
      console.error("❌ reorder-check refresh threw after purchase", {
