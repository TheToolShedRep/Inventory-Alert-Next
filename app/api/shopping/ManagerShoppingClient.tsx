...
export default function ManagerShoppingClient() {
  ...
  async function refresh() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/shopping-list?includeHidden=0", {
        method: "GET",
        cache: "no-store",
      });
  ...
  async function act(
    ...
    try {
      const res = await fetch("/api/shopping/action", {
        ...
        // ✅ matches your current route payload: { upc, action, note? }
    ...
    <div style={{ padding: 16, maxWidth: 900 }}>
      ...
      <div style={{ marginTop: 16, display: "grid", gap: 10 }}>
        ...
        {!loading && sorted.length === 0 ? (
          <div style={{ marginTop: 10, opacity: 0.8 }}>No items right now.</div>
        ) : null}
      </div>
    </div>
  );
